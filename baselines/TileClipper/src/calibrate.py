...
class Calibrate():
    def __init__(self, tiled_video_directory: str, start_percentile: int = 10, end_percentile: int = 81, number_of_calibration_segments: int = 60, total_tiles: int = 16, cluster_size: int = 10, save_name = "name"):
    ...
    def main(self, bitrates: np.ndarray, ground_truth_from_yolov5: str, folder_name_to_save_in: str):
        ...
        jb.dump(meanFScore, f"{str(Path(folder_name_to_save_in))}/f2s_{self.save_name}_cluster{self.clusterSize}.pkl")
        jb.dump(clusterIndices, f"{str(Path(folder_name_to_save_in))}/{self.save_name}_cluster_indices.pkl")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save-name", required=True, type=str, help="save name")
    ...
    groundTruthFile = f"{opt.assets_folder}/GroundTruths_TileLevel/{opt.save_name}.txt"
