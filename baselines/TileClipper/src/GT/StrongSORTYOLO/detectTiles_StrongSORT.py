...
def run(
    ...
    save_label_path = save_labelfolder_name

    if os.path.exists(save_label_path):
        os.remove(save_label_path)
    ...
    for frame_idx, (path, im, im0s, vid_cap, s) in enumerate(dataset):
        ...
        # print("segment: ", segment, "frame_idx: ", frame_idx, "totalFrames: ", totalFrames, ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
        t1 = time_sync()
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                confs = det[:, 4]
                clss = det[:, 5]
                #for j, (xywh, conf, cls) in enumerate(zip(xywhs, confs, clss)):
                #    print(f"Detected {names[int(cls)]} with confidence {conf:.2f} at {xywh}")
                ...
                if len(outputs[i]) > 0:
                    frameIndex = (frame_idx % totalFrames) + 1
                    ...
                    if frameIndex > 2 and frameIndex < totalFrames:
                        ...
                        with open(save_label_path, 'a') as t:
                            for d in detect_1:
...
def parse_opt():
    ...
    parser.add_argument('--save-labelfolder-name', type=str, help='Folder (with "/") where to save label.txt file')
